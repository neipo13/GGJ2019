...
{
    public class SimpleScreenShake : Component, IUpdatable
    {
        ...
        public SimpleScreenShake(Vector2 originalPosition)
        {
        ...
        public void shake(float shakeIntensity = 15f, float shakeDegredation = 0.9f, float shakeTime = 0.3f, Vector2 shakeDirection = default(Vector2))
        {
            enabled = true;
            ...
            if (_shakeIntensity < shakeIntensity)
            {
                ...
                entity.scene.camera.position = originalPosition;
            }
        }
        ...
        void IUpdatable.update()
        {
            if (_shakeTime > 0f)
            {
                ...
                if (Math.Abs(_shakeIntensity) <= 0.1f || _shakeTime <= 0f)
                {
                    _shakeIntensity = 0f;
            ...
            entity.scene.camera.position = originalPosition + _shakeOffset;
            entity.scene.camera.position = Vector2Ext.approach(entity.scene.camera.position, originalPosition, Vector2Ext.abs(_shakeOffset / 3));
        }
    }
}
